...
local function read_queries(filename)
  ...
  for line in f:lines() do
    ...
    elseif starts_with(trimmed, "--") then
      ...
      if current then table.insert(sql_lines, line) end
    end
    ::continue::
  end
  flush()
...
local function SQL(str, dialect)
  ...
  local sql = str:gsub(":::([%w_]+)", function(name)
...
return function(db)
  ...
  for _, q in ipairs(queries) do
    ...
    if #order == 0 then
      ...
      table.insert(lines, string.format("    local order = %s", table_to_string(order)))
